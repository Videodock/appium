...
IOS.prototype.startRealDevice = function (cb) {
  async.series([
    ...
    this.launchRemoteDebugProxy.bind(this),
...
IOS.prototype.launchRemoteDebugProxy = function (cb) {
  // launch if is udid is passed

  logger.info('ios_webkit_debug_proxy: test if start is needed');
  ...
  if (this.args.udid !== null) {
    ...
    this.wkrdProcess.stdout.on('data', function (data) {
      console.log('' + data);
    });
    setTimeout(cb, 5000);
    return;
  ...
  logger.info('ios_webkit_debug_proxy: not needed');
