...
iOSHybrid.setCurrentUrl = function (url) {
  this._currentUrl = url;
};

iOSHybrid.getCurrentUrl = function () {
...
iOSHybrid.listWebFrames = function (cb, exitCb) {
  ...
  if (this.remote !== null && this.args.bundleId !== null && this.remoteAppKey) {
    if (this.args.udid !== null) {
      this.remote.pageArrayFromJson(onDone);
    } else {
      this.remote.selectApp(this.remoteAppKey, this.getCurrentUrl(), function (err, perhapsModifiedAppKey, res) {
    ...
    if (this.args.udid !== null) {
      ...
      // start webkit remote debugger
      logger.info('ios_webkit_debug_proxy: spawning child process');
      this.wkrdProcess = spawn('ios_webkit_debug_proxy', ['-d', '-c', this.args.udid + ':27753']);
      ...
      this.remote.connect(function (appDict) {
        ...
        this.remote.selectApp(appKey, this.getCurrentUrl(), function (err, perhapsModifiedAppKey, res) {
          if (err) return onDone(err);
          this.remoteAppKey = perhapsModifiedAppKey;
          onDone(null, res);
        }.bind(this), this.args.webviewConnectRetries);
...
iOSHybrid.onPageChange = function (pageArray) {
  ...
  if (!_.isUndefined(this.curContext) && !_.isNull(this.curContext)) {
    var page = _.find(pageArray, function (p) {
...
iOSHybrid.stopRemote = function (closeWindowBeforeDisconnecting) {
  ...
  if (!this.remote) {
    ...
    var disconnect = function () {
      ...
      this.remote = null;
      if (this.wkrdProcess) {
        this.wkrdProcess.kill('SIGHUP');
        delete this.wkrdProcess;
        logger.info('ios_webkit_remote_debugger: stopped process');
      }
    }.bind(this);
